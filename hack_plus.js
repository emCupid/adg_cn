...
(function() {
    //if ($$ALL_WLD.indexOf(getDoamin) == -1 && !$$ALL_WLD.some(Fuck_testDomain)) {
        ...
        window.Timer_FuckRAI = setTimeout(function () {
            Fuck_ADV();
            //[].forEach.call(document.querySelectorAll('head>style'),function(Item) {
            //    //console.log(Item.innerText);
            //    if (whiteRegex.test(Item.innerText)) {
            //        Item.innerText = Item.innerText.replace(whiteRegex,'{display:block!important;max-height:none!important;max-width:100%!important;overflow:auto!important;position:relative;left:auto}')
            //    }
            //});
